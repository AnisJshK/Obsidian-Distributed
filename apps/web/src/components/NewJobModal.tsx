...
const TASK_TEMPLATES: Record<string, { label: string; defaultPayload: object }> = {
  "send-email": {
    ...
    defaultPayload: {
      ...
      body: "All background worker systems operational.",
    },
  },
  "resize-image": {
    label: "Resize Image (Task)",
    defaultPayload: {
      task: "resize-image",
  ...
  "flaky-task": {
    label: "Flaky Task (Fails 2x, Tests Retry & DLQ)",
    defaultPayload: {
      task: "flaky-task",
      failureRate: 0.8,
      jobContext: "Testing exponential backoff + DLQ cascade",
    },
  },
  "custom": {
...
export const NewJobModal: React.FC<NewJobModalProps> = ({ queues, onClose }) => {
  ...
  const [payloadText, setPayloadText] = useState(
    JSON.stringify(TASK_TEMPLATES["send-email"].defaultPayload, null, 2)
  );
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-xs">
      <div className="w-full max-w-xl bg-[#0b1120] border-l border-[#162033] h-full overflow-y-auto p-6 space-y-6 shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-[#151e30] pb-4">
          <h2 className="text-sm font-bold text-white font-sans">Enqueue New Job</h2>
          <button
            onClick={onClose}
        ...
        {/* Task Template Quick-Fill */}
        <div className="space-y-1.5">
          <label className="text-[11px] uppercase font-semibold text-slate-500 flex items-center gap-1.5">
        ...
        <div className="space-y-2">
          ...
          {timingMode === "SCHEDULED" && (
            <input
              ...
              className="w-full bg-[#070b14] border border-[#162033] rounded-lg px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-blue-500 mt-2"
            />
          )}
        </div>
        ...
        {/* Advanced Execution & Dependency Controls */}
        <div className="space-y-3">
          <button
            onClick={() => setShowAdvanced(!showAdvanced)}
            className="text-[11px] uppercase font-semibold text-slate-400 hover:text-slate-200 transition"
          >
            {showAdvanced ? "▾ Hide" : "▸ Show"} Advanced Options & Dependencies
          </button>
            ...
            <div className="bg-[#070b14] border border-[#151e30] rounded-xl p-4 space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] uppercase font-semibold text-slate-500">
                  Parent Dependencies (parent job IDs, comma-separated)
                </label>
                <input
                  type="text"
