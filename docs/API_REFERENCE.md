...
The authentication router is mounted under `/api/auth`.
...
Validates the request with `RegisterUserSchema` and creates a user account.
...
Validates the request with `LoginSchema` and authenticates the user.
...
Creates a new API key for an authenticated user/project context.
...
Returns API keys available to the authenticated user.
...
Job-related routes are mounted under \`/api/v1/jobs\`.
...
GET /api/v1/jobs?status=QUEUED&queueName=default
...
Worker routes are mounted under \`/api/v1/worker\`.
...
The API intentionally separates user-facing management operations from
programmatic project-scoped operations:

| Route group | Authentication | Purpose |
|---|---|---|
| `/api/auth/register` | Public | User registration |
| `/api/auth/login` | Public | User login |
| `/api/auth/me` | User session | Current user |
| `/api/auth/register-project` | User session | Project onboarding |
| `/api/auth/logout` | Session | Clear user session |
| `/api/auth/verify` | Request validation | API key verification |
| `/api/auth/session` | API key | Current project context |
| `/api/projects` | User session | Project management |
| `/api/keys` | User session | API key management |
| `/api/queues` | User/project context | Queue management |
| `/api/dlq` | User/project context | DLQ operations |
| `/api/workflows` | User/project context | Workflow operations |
| `/api/schedules` | User/project context | Recurring schedules |
| `/api/v1/jobs` | API key | Job submission and management |
| `/api/v1/worker` | API key | Worker-facing operations |
...
\| \`POST\`   | \`/api/v1/jobs/batch\`            | Enqueue jobs in batch     |
...
\| \`GET\`    | \`/api/v1/worker\`               | List workers              |
...
Authorization: Bearer djs\_live\_xxx
